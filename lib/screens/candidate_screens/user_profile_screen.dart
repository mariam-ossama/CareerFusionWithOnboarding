...
class _MyWidgetState extends State<UserProfilePage> {
  ...
  Future<void> uploadProfilePicture() async {
    ...
    try {
      final url = 'http://10.0.2.2:5266/api/UserProfile/upload-profile-picture/${userId}';
  ...
  Future<void> fetchProfilePicture() async {
     ...
     String? userId = prefs.getString('userId');
    try {
      final url = 'http://10.0.2.2:5266/api/UserProfile/${userId}/profile-picture';
  ...
  setState(() {
    ...
    _projects = (responseBody['projectLinks'] as List)
            .map((project) => Project(
                  projectId: project['projectLinkId'], // Include projectId here
  ...
  Future<void> deleteSiteLink(String siteLinkId) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? userId = prefs.getString('userId');
    final url = 'http://10.0.2.2:5266/api/UserProfile/$userId/sitelinks/$siteLinkId';
    try {
      final response = await http.delete(Uri.parse(url));
      if (response.statusCode == 200) {
        setState(() {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('An error occurred while deleting the site link')),
      );
    }
  }
  ...
  Future<void> downloadCV() async {
    try {
      final response =
          await http.get(Uri.parse('http://10.0.2.2:5266/api/UserProfile/download-file'));
    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          ...
          Padding(
            ...
            child: Card(
              ...
              child: Column(
                children: [
                  ListTile(
                    title: Text('Description', style: TextStyle(//fontFamily: appFont,
          ...
          Padding(
            ...
            child: Card(
              ...
              child: Column(
                children: [
                  ListTile(
                    title: Text('Address', style: TextStyle(//fontFamily: appFont,
                     fontWeight: FontWeight.bold),),
                    subtitle: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [Text(_address ?? 'No address', style: TextStyle(//fontFamily: appFont
                      ),),],),
                  ),
                  // Additional content
                ],
          ...
          Center(child: Text('My Courses', style: TextStyle(//fontFamily: appFont,
...
Padding(
  ...
  SharedPreferences prefs = await SharedPreferences.getInstance();
  String? userId = prefs.getString('userId');
  final url = 'http://10.0.2.2:5266/api/UserProfile/$userId/courses/${course.courseId}';
  try {
    final response = await http.delete(Uri.parse(url));
    print(response.body);
    print(response.statusCode);
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('An error occurred while deleting the course')),
    );
  }
},
          ),
...
Divider(
          ...
          Center(child: Text('My Projects', style: TextStyle(//fontFamily: appFont,
           fontSize: 16,fontWeight: FontWeight.bold),)),
    ...
    itemBuilder: (context, index) {
        ...
        child: ListTile(
          ...
          trailing: IconButton(
            ...
            onPressed: () async {
              SharedPreferences prefs = await SharedPreferences.getInstance();
              String? userId = prefs.getString('userId');
              final url = 'http://10.0.2.2:5266/api/UserProfile/$userId/projectlinks/${project.projectId}';
              try {
                final response = await http.delete(Uri.parse(url));
                print(response.body);
                print(response.statusCode);
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('An error occurred while deleting the project')),
                );
              }
            },
          ),
          ...
          Center(child: Text('My Skills', style: TextStyle(//fontFamily: appFont,
           fontSize: 16,fontWeight: FontWeight.bold),)),
          Padding(
            padding: const EdgeInsets.all(8.0),
            child: ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Padding(
          ...
          Center(child: Text('My Sitelinks', style: TextStyle(//fontFamily: appFont,
           fontSize: 16,fontWeight: FontWeight.bold),)),
Padding(
  padding: const EdgeInsets.all(8.0),
  child: ListView.builder(
...
Padding(
  ...
  child: Card(
    ...
    child: Column(
      children: [
        ListTile(
  leading: IconButton(
  icon: Icon(Icons.file_download, color: mainAppColor,), // Icon for download
  onPressed: () async {
    /*try {
  ...
  title: Text('My CV', style: TextStyle(//fontFamily: appFont,
  fontWeight: FontWeight.bold)),
  subtitle: Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('CV', style: TextStyle(//fontFamily: appFont
      )),
    ],
  ),
  trailing: IconButton(
    icon: Icon(Icons.file_upload,color: mainAppColor,), // Icon for upload
    onPressed: () async {
    try {
      ...
      if (result != null) {
        ...
        var request = http.MultipartRequest('POST', Uri.parse('http://10.0.2.2:5266/api/UserProfile/upload-file'));
        request.files.add(
          await http.MultipartFile.fromPath(
            'file',
            file.path,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('An error occurred while uploading CV')),
      );
    }
  },
  ),
...
SizedBox(height: 10,),
CustomButton(text: 'Edit your profile',
onPressed: (){
