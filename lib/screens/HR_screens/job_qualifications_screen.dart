...
class _JobQualificationsPageState extends State<JobQualificationsPage> {
  ...
  Future<void> fetchJobSkills() async {
    ...
    if (response.statusCode == 200) {
      ...
      final List<dynamic> skillIds = jobDetails['jobSkills']; // Extract description IDs
      setState(() {
        jobSkills = skills.map<String>((d) => d['skillName'] as String).toList();
        jobSkillIds = skillIds.map<int>((d) => d['id'] as int).toList(); // Store description IDs
      });
    } else {
  ...
  String? newSkill = await showDialog(
    ...
    builder: (context) => AlertDialog(
      ...
      content: TextField(
        controller: newSkillController, // Use the controller to retrieve entered text
  ...
  SharedPreferences prefs = await SharedPreferences.getInstance();
  String? userId = prefs.getString('userId');
  final url = '${baseUrl}/JobForm/updateSkill/$userId/${widget.jobId}/$skillId';
  final response = await http.put(
    Uri.parse(url),
    body: jsonEncode({'id': skillId, 'skillName': newSkill}), // Use newDescription
...
Future<void> DeleteJobSkill(int skill) async {
  SharedPreferences prefs = await SharedPreferences.getInstance();
  String? userId = prefs.getString('userId');
  final url = '${baseUrl}/JobForm/DeleteJobSkill/$userId/${widget.jobId}/$skill';
  ...
  itemBuilder: (context, index) {
    return JobSkillTile(
      ...
      onDelete: (skillId) => DeleteJobSkill(skillId), // Pass onDelete function
    );
  },
),
    );
      ...
      trailing: Row(
        ...
        children: [
          IconButton(
            ...
            onPressed: () => onEdit(skillId), // Pass responsibilityId to onEdit function
          ),
          IconButton(
            icon: Icon(Icons.delete),
            onPressed: () => onDelete(skillId), // Pass responsibilityId to onDelete function
          ),
        ],
      ),
    );
