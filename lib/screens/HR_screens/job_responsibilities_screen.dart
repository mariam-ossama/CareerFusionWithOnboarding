...
class JobResponsibilitiesPage extends StatefulWidget {
  final String userId;
  final int jobId;
  JobResponsibilitiesPage(
      {super.key, required this.userId, required this.jobId});

  @override
  State<JobResponsibilitiesPage> createState() =>
      _JobResponsibilitiesPageState();
}
...
class _JobResponsibilitiesPageState extends State<JobResponsibilitiesPage> {
  ...
  final TextEditingController newResponsibilityController =
  ...
  Future<void> fetchJobResponsibilities() async {
    ...
    if (response.statusCode == 200) {
      ...
      final List<dynamic> responsibilityIds =
          jobDetails['jobResponsibilities']; // Extract description IDs
      setState(() {
        jobResponsibilities = responsibilities
            .map<String>((d) => d['responsibility'] as String)
            .toList();
        jobResponsibilityIds = responsibilityIds
  ...
  Future<void> editJobResponsibility(int index, int responsibilityId) async {
    String? newResponsibility = await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(
          controller:
              newResponsibilityController, // Use the controller to retrieve entered text
  ...
  Future<String> updateResponsibility(
      String newResponsibility, int responsibilityId) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? userId = prefs.getString('userId');
    final url =
        '${baseUrl}/JobForm/updateResponsibility/$userId/${widget.jobId}/$responsibilityId';
  ...
  Future<void> DeleteJobResponsibility(int responsibility) async {
    ...
    String? userId = prefs.getString('userId');
    final url =
        '${baseUrl}/JobForm/DeleteJobResponsibility/$userId/${widget.jobId}/$responsibility';
      ...
      body: ListView.separated(
        ...
        itemBuilder: (context, index) {
          return JobResponsibilityTile(
            ...
            responsibilityId:
                jobResponsibilityIds[index], // Pass description ID
            onEdit: (responsibilityId) =>
                editJobResponsibility(index, responsibilityId),
            onDelete: (responsibilityId) => DeleteJobResponsibility(
                responsibilityId), // Pass onDelete function
          );
        },
      ),
    );
      ...
      trailing: Row(
        ...
        children: [
          IconButton(
            ...
            onPressed: () => onEdit(
                responsibilityId), // Pass responsibilityId to onEdit function
          ),
          IconButton(
            icon: Icon(Icons.delete, color: mainAppColor),
            onPressed: () => onDelete(
                responsibilityId), // Pass responsibilityId to onDelete function
          ),
        ],
      ),
    );
